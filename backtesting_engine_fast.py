#!/usr/bin/env python3
"""
Optimized Fast Backtesting Engine
Major performance improvements over the original backtesting engine
"""
...
class FastBacktestingEngine:
    """Optimized backtesting engine with major performance improvements"""
    ...
    def generate_trading_signal_fast(self, current_indicators: Dict, current_price: float) -> Dict:
        """Fast signal generation using pre-calculated indicators"""
        signal = {
            'action': 'HOLD',
            'strength': 0.0,
            'indicators_used': []
        ...
        # Simple signal logic (can be made more sophisticated)
        signal_strength = 0.0
        
        # RSI signal
        if 'rsi' in current_indicators and not pd.isna(current_indicators['rsi']):
            rsi = current_indicators['rsi']
            if rsi < 30:
                signal_strength += 0.3
                signal['indicators_used'].append('rsi')
            elif rsi > 70:
                signal_strength -= 0.3
                signal['indicators_used'].append('rsi')
        ...
        if 'macd' in current_indicators and 'macd_signal' in current_indicators:
            ...
            if not pd.isna(macd) and not pd.isna(macd_signal):
                if macd > macd_signal:
                    signal_strength += 0.2
                    signal['indicators_used'].append('macd')
                else:
                    signal_strength -= 0.2
                    signal['indicators_used'].append('macd')
        
        # Bollinger Bands signal
        if 'bb_upper' in current_indicators and 'bb_lower' in current_indicators:
            bb_upper = current_indicators['bb_upper']
            bb_lower = current_indicators['bb_lower']
            if not pd.isna(bb_upper) and not pd.isna(bb_lower):
                if current_price <= bb_lower:
                    signal_strength += 0.25
                    signal['indicators_used'].append('bollinger_bands')
    ...
    def calculate_position_size_fast(self, balance: float, entry_price: float, 
    ...
    def apply_execution_costs_fast(self, price: float, direction: str, volume: float) -> Tuple[float, float]:
        """Fast execution cost calculation"""
    ...
    def execute_trade_fast(self, symbol: str, direction: str, price: float, 
                          timestamp: datetime.datetime, volume: float) -> Trade:
        """Fast trade execution"""
        # Apply execution costs
        execution_price, commission = self.apply_execution_costs_fast(price, direction, volume)
        
        # Calculate stop loss and take profit
        stop_loss_distance = 20 * 0.01  # 20 pips for Gold
        take_profit_distance = 40 * 0.01  # 40 pips for Gold
    ...
    def close_trade_fast(self, trade: Trade, exit_price: float, exit_time: datetime.datetime, 
    ...
    def run_backtest_fast(self, symbol: str, start_date: datetime.datetime = None, 
                         end_date: datetime.datetime = None, initial_balance: float = 10000) -> BacktestResults:
        """Fast backtesting with major performance optimizations"""
        
        print(f"🚀 FAST BACKTESTING MODE")
        ...
        for i in range(min_bars, len(df)):
            ...
            # Generate trading signal (fast version)
            ...
            if symbol in self.current_positions:
                ...
                if exit_reason:
                    closed_trade = self.close_trade_fast(trade, exit_price, timestamp, exit_reason)
            ...
            if symbol not in self.current_positions and signal['action'] != 'HOLD':
                # Calculate position size
                risk_per_trade = self.config['risk_management']['position_sizing']['risk_per_trade']
                volume = self.calculate_position_size_fast(self.current_balance, current_price, 
                                                         current_price * 0.98, risk_per_trade)
                
                if volume > 0:
                    # Execute trade
                    direction = 'LONG' if signal['action'] == 'BUY' else 'SHORT'
                    trade = self.execute_trade_fast(symbol, direction, current_price, timestamp, volume)
                    trade.indicators_used = signal['indicators_used']
        ...
        for trade in self.current_positions.values():
            ...
            closed_trade = self.close_trade_fast(trade, final_price, final_time, 'END_OF_DATA')
            self.trades_history.append(closed_trade)
        ...
        # Calculate results
        results = self.calculate_results_fast(initial_balance, df.index[0], df.index[-1])
    ...
    def calculate_results_fast(self, initial_balance: float, start_date: datetime.datetime, 
        ...
        # Maximum drawdown - optimized calculation
...
def main():
    """Demo function to test fast backtesting"""
    print("🚀 FAST BACKTESTING ENGINE DEMO")
    print("=" * 50)
    
    engine = FastBacktestingEngine()
    
    # Run fast backtest
    results = engine.run_backtest_fast(
        symbol="15",
